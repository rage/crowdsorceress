...
class SubmissionStatusChannel < ApplicationCable::Channel
  ...
  def submission_state(exercise)
    if exercise.blank?
      ...
      state = { 'status' => 'in progress', 'message' => 'Tehtävä tallennettu tietokantaan', 'progress' => 0.1, 'result' => { 'OK' => false, 'error' => '' } }
    elsif exercise.testing_stub?
      state = { 'status' => 'in progress', 'message' => 'Testataan tehtäväpohjaa', 'progress' => 0.3, 'result' => { 'OK' => false, 'error' => '' } }
    elsif exercise.testing_model_solution?
      state = { 'status' => 'in progress', 'message' => 'Testataan malliratkaisua', 'progress' => 0.5, 'result' => { 'OK' => false, 'error' => '' } }
